...
def johnson_champoux(flow_resis: float, air_dens: float, poros: float,
    ...
    if var == 'default':
        (zc, kc) = __default(flow_resis, air_dens, poros, tortu, gama,
                             prandtl, atm, visc, therm, neta, freq)

        return (zc, kc)

    if var == 'allard':
        (zc, kc) = __allard(flow_resis, air_dens, poros, tortu, gama,
                            prandtl, atm, visc, therm, neta, freq)

        return (zc, kc)

    if var == "lafarge":
        (zc, kc) = __lafarge(flow_resis, air_dens, poros, tortu, gama, atm,
                             visc, therm, neta, freq, therm_perm, Cp)
...
def __lafarge(flow_resis: float, air_dens: float, poros: float, tortu: float,
              gama: float, atm: float, visc: float, therm: float, neta: float,
              freq: np.ndarray, therm_perm: float = 0,
              Cp: float = 0) -> Tuple[np.ndarray, np.ndarray]:
    """Returns through the Johnson-Champoux-Allard-Lafarge Model the Material
...
def __allard(flow_resis: float, air_dens: float, poros: float, tortu: float,
             gama: float, prandtl: float, atm: float, visc: float,
             therm: float, neta: float,
             freq: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """Returns through the Johnson-Champoux-Allard Model the Material
    Charactheristic Impedance and the Material Wave Number.
...
def __default(flow_resis: float, air_dens: float, poros: float, tortu: float,
              gama: float, prandtl: float, atm: float, visc: float,
              therm: float, neta: float,
              freq: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """Returns through the Johnson-Champoux Model the Material Charactheristic
